...
test("pass custom id to round", t => {
  const r = newGame(table1)
    .update(actions => actions.newRound({id: "custom-id", deck: randomDeck()}))
    .get()

  t.deepEqual(r.round.id, "custom-id")
})
...
test("2-players: leavePlayer", t => {
  ...
  const r = newGame(table)
    .update(actions => actions.newRound({deck: randomDeck()}))
